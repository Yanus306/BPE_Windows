    ...
    public static class HttpRedirectChainAnalyzer
    {
        public static async Task<RedirectChainResult> AnalyzeAsync(string url, int maxRedirects = 10)
        {
            ...
            while (redirectCount < maxRedirects)
            {
                ...
                result.Steps.Add(step);
                // 리다이렉션 여부
                if (resp.StatusCode is HttpStatusCode.Moved or HttpStatusCode.Redirect or HttpStatusCode.RedirectMethod or HttpStatusCode.TemporaryRedirect or (HttpStatusCode)308)
                {
