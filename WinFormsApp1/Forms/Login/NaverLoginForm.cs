    ...
    public class NaverLoginForm : Form
    {
        ...
        private async Task<(bool success, string? accessToken, string? error)> ExchangeCodeForToken(string code, string state)
        {
            try
            {
                ...
                var content = new StringContent($"grant_type=authorization_code&client_id={ClientId}&client_secret={ClientSecret}&code={code}&state={state}", Encoding.UTF8, "application/x-www-form-urlencoded");
